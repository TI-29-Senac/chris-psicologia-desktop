    ...
    async sincronizacaoBidirecional() {
        try {
            ...
            for (const user of pendentesLocais) {
                try {
                    // SE FOR PROFISSIONAL, PRECISAMOS PEGAR OS DADOS DA TABELA FILHA
                    let dadosParaEnvio = { ...user };

                    // FIX: Adiciona 'id' como alias para 'id_usuario'
                    dadosParaEnvio.id = user.id_usuario;

                    if (user.tipo_usuario === 'profissional' && !user.excluido_em) {
                        const dadosProf = db.prepare('SELECT especialidade, valor_consulta, sinal_consulta FROM profissional WHERE id_usuario = ?').get(user.id_usuario);
                        if (dadosProf) {
                            dadosParaEnvio = { ...dadosParaEnvio, ...dadosProf };
                            console.log("Anexando dados de profissional ao payload:", dadosProf);
                        }
                    }
